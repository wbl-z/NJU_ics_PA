...
这个函数的作用是把GDT从低地址搬到高地址中，即C000000+GDT的地址，因为在kernel的页表初始化时是进行了低地址30000和高地址c0030000到物理地址30000的映射，所以kernel中使用30000的GDT是没问题的，是映射到实际物理地址中的，而用户页表仅复制了kernel页表的高地址部分，因此如果GDTR中还是低地址的话，用户是无法访问GDT的，因此需要将GDTR改成高地址的，这样才能访问到实际物理地址30000的GDT*/
	init_segment();
